...
## 사용법
1. application-secret.yml 에 들어가 이메일 입력하고 gmail에 서 2차인증 켜주고 앱 비밀번호 발급해서 16자리 발급받음
2. application-secret.yml 에 비밀 번호입력하기
3. AsciiSeed 에서 이메일 부분 본인 이메일로 수정
4. 회원가입후 다시 서버 로딩하면 테스트 데이터 생성
...
이메일 인증을 요청하면 처음에는5초가량 거릴다 이제는 13초나 걸리는문제

## 원인

Gmail SMTP 사용해서 서버와 연결해 메일을 발송하는 방식을 사용했는데 
서버 상태가 좋지 않으면 느려지는 문제였음

## 해결

구간별 처리 시간을 로그로 측정하여 병목 지점을 확인했다.
System.out.println("redis=" + (t1 - t0) + "ms, mail=" + (t2 - t1) + "ms");
측정 결과 redis=0ms, mail=13931ms로 Redis 저장이 아니라 SMTP 전송(JavaMailSender.send) 구간이 병목임을 확인했다.
통제를 벗어난 행위 임으로 체감속도를 개선해 해결함
(향후 운영 환경에서는 Gmail SMTP 대신 전용 메일 전송 서비스(예: SES/SendGrid/Mailgun) 도입을 고려)
## 배운 점

성능 문제는 추측이 아니라 측정(로그/구간 분리) 으로 병목을 찾아야 한다.

외부 의존(SMTP)처럼 통제 어려운 영역은 비동기/큐 설계로 UX를 보호하는 게 중요하다.

## 5. 인증번호가 5분이 지나도 만료되지 않는 문제

## 문제

인증번호가 5분이 지나도 사용가능한 문제

## 원인

dev 환경에서 MAP 에 저장하니 기록이 영구 저장되는 문제원인

## 해결

TTL(Time To Live) 를 사용 하여
...
## 6. 실행했을떄 잘돌아가던 코드가 먹통이됨

## 문제

컴푸터 업데이트와 파일 정리이후 부터 코드가 작동되지않는 문제

## 원인

의존성주입 문제로 코드가 제대로 의존성을 받지못해서 생긴 문제였다

## 해결

 Gradle 에서 새로고침을 해서 의존성을 다시 부여했다 

## 배운 점

컴퓨터 업데이트는 신중해야하며 파일정리는 더욱 신중해야한다 이러한 사소한 문제로 서비스가 준단될수 있고 재부팅으로 해결할수도 없다




지금 발견된 문제 로인된 상태로 다른 유저의 비공개 게시물에 접근하려하면 로그인 창으로 가버리는 문제
비로그인 이라면 정상이지만 로그인상태로 하면 안됨
